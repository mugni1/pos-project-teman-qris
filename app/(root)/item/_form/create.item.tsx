...
export default function CreateItem({ getParams }: { getParams: GetParams }) {
  const queryClient = useQueryClient();
  const { mutateAsync: createMutate } = useCreateItem();
  ...
  const onSubmit = async (values: CreateItemPayload) => {
    try {
      ...
      const payload: CreateItemPayloadService = {
      ...
      >
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <PlusCircleIcon /> Tambah Item Baru
          </DialogTitle>
          <DialogDescription>
            Lengkapi data item, lalu simpan untuk menambahkan item baru ke
            sistem.
          </DialogDescription>
        </DialogHeader>
